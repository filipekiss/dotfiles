function vim-gen-related() {
    ...
    echo "\" Related Files ---------------------------------------- {{{" | tee /tmp/vim-related-files > /dev/null
    echo ${(iF)__vim_related_files} | tee -a /tmp/vim-related-files > /dev/null
    echo "\" }}} ---------------------------------------- /Related Files" | tee -a /tmp/vim-related-files > /dev/null
    ...
    sed -i.bak -e '/" Related Files/,/Related Files/d' $DOTFILES/config/vim/.vimrc-pre-insert-backup
    ...
    # Ensure Related Files is correctly in place (that means only 2 'Related Files' references)
    local __other_files_ref_count=$(grep -ir "Related Files" $DOTFILES/config/vim/.vimrc-pre-insert-backup | wc -l)
